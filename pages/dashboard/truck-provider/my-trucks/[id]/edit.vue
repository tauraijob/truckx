  ...
  <div class="min-h-screen bg-gray-50 py-12">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
      <div class="mb-8 flex justify-between items-center">
        <div>
          <h1 class="text-2xl font-semibold text-gray-900">Edit Truck</h1>
          <p class="mt-1 text-sm text-gray-600">Update your truck information and images</p>
        </div>
        <button
          @click="goBack"
      ...
      <div v-else class="bg-white shadow rounded-lg">
        <div class="p-6">
          <form @submit.prevent="submitForm">
            ...
            <div v-if="currentImages.length > 0" class="mb-8">
              ...
              <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                <div v-for="(img, index) in currentImages" :key="index" class="relative group">
                  <img :src="img" :alt="`Truck image ${index + 1}`" class="h-32 w-full object-cover rounded-lg" />
                  <div class="absolute bottom-0 left-0 right-0 bg-black bg-opacity-50 text-white text-xs py-1 px-2">
                    {{ index === 0 ? 'Featured' : `Gallery ${index}` }}
                  </div>
                </div>
              </div>
              ...
              <div class="mt-4 flex items-center">
                <input 
                  ...
                  type="checkbox" 
                  v-model="keepExistingImages"
                  class="h-4 w-4 text-primary-600 focus:ring-primary-500 border-gray-300 rounded"
                />
                <label for="keepExistingImages" class="ml-2 block text-sm text-gray-700">
            ...
            <div class="mb-8">
              <h2 class="text-lg font-semibold mb-4">Update Featured Image</h2>
              <p class="mb-4 text-sm text-gray-600">Upload a new main image that will be displayed prominently for your truck</p>
            ...
            <div class="mb-8">
              <h2 class="text-lg font-semibold mb-4">Update Gallery Images</h2>
              <p class="mb-4 text-sm text-gray-600">Add or replace additional images to showcase your truck (up to 7 images)</p>
...
// Current truck images
const currentImages = ref<string[]>([])
const keepExistingImages = ref(true)
...
onMounted(async () => {
  try {
    ...
    truckData.value = {
      ...
      type: truck.type || 'Semi-Trailer',
      capacity: truck.capacity || 0,
      licensePlate: truck.licensePlate || '',
      currentLocation: truck.currentLocation || '',
      isAvailable: truck.isAvailable !== undefined ? truck.isAvailable : true
    }
    
    // Set current images
    if (truck.images && truck.images.length > 0) {
      currentImages.value = truck.images as string[]
    }
    
    // Set specifications
    if (truck.specifications) {
      const specs = truck.specifications as Record<string, any>
      specifications.engineType = specs.engineType || ''
      specifications.transmission = specs.transmission || ''
      specifications.fuelType = specs.fuelType || ''
      specifications.mileage = specs.mileage || null
      specifications.additionalInfo = specs.additionalInfo || ''
    }
...
const validateForm = () => {
  ...
  // No need to validate images for edit, as we already have existing images
...
const submitForm = async () => {
  ...
  try {
    ...
    const submitData = {
      ...
      keepExistingImages: keepExistingImages.value
    }
    
    // Add images if they were uploaded
    if (featuredImage.value) {
      submitData.featuredImage = featuredImage.value
    }
    
    if (galleryImages.value.length > 0) {
      submitData.galleryImages = galleryImages.value
    }
    ...
    if (error.response && error.response._data) {
      alert(`Error: ${error.response._data.message || 'Failed to update truck'}`)
    } else {

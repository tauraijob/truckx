...
export default defineEventHandler(async (event) => {
    console.log('Fetching outstanding fees for truck provider')
    ...
    try {
        ...
        // Fetch the truck provider with their outstanding fees and orders
        ...
        if (provider.truckProviderOrders && provider.truckProviderOrders.length > 0) {
            calculatedOutstandingFees = provider.truckProviderOrders.reduce((total, order) => {
        ...
        return {
            id: provider.id,
            outstandingAmount,
            trucksCount: provider._count.trucks,
            lastPaymentDate: provider.payments.length > 0 ? provider.payments[0].createdAt : null,
            status,
            completedOrdersCount: provider.truckProviderOrders?.length || 0
        }
    } catch (error) {
        console.error('Error fetching truck provider fees:', error)
        throw createError({
            statusCode: 500,
